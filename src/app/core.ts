...
export interface App {
   ...
   * print(app.options.credential === config.credential);  // true
   * print(app.options.databaseURL === config.databaseURL);  // true
   * ```
   */
  options: AppOptions;
}
 ...
 *       print("Following devices unsucessfully subscribed to topic:");
 *       response.errors.forEach(function(error) {
 *         var invalidToken = registrationTokens[error.index];
 *         print(invalidToken, error.error);
 *       });
 *     } else {
 *       print("All devices successfully subscribed to topic:", response);
 *     }
 *   })
